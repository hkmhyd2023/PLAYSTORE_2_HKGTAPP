...
class _EditProfileState extends State<EditProfile> {
  ...
  String ashrayaLevelValue = "Select Ashraya Level";
  String preacherValue = 'Select Japa Coordinator';
  String giftCenterValue = 'Select Gifts Center';
 List<DropdownMenuItem<String>> ashrayaLevel = [ DropdownMenuItem<String>(
                            value: "Sharadhavan",
                            child: Text("Sharadhavan"),
...
FirebaseMessaging.instance.getToken().then((token) {
                  ...
                  Padding(
                    ...
                    child: Container(
                      decoration: boxDecorationWithRoundedCorners(
                        ...
                        border: Border.all(color: Colors.orange),
                        backgroundColor:
                            false ? cardDarkColor : const Color(0xFFF8F8F8),
                      ),
  ...
  builder: (context, snapshot) {
    ...
    final List<DocumentSnapshot> preachers = snapshot.data!.docs.toList();
    List<DropdownMenuItem<String>> ashrayaLevel = [];

    for (var preacher in preachers) {
      ashrayaLevel.add(DropdownMenuItem<String>(
        value: preacher['Name'].toString(),
        child: Text(preacher['Name'].toString()),
      ));
    }

    if (ashrayaLevel.isEmpty) {
      // Handle case where there are no items in the list
      return Text('No Ashraya levels found');
    }

    // Ensure that the value of ashrayaLevelValue is one of the valid values
    if (!ashrayaLevel.map((item) => item.value).contains(ashrayaLevelValue)) {
      // Set ashrayaLevelValue to the first item in the list
      ashrayaLevelValue = ashrayaLevel.first.value!;
    }
    ...
    return Padding(
      ...
      child: Container(
        decoration: boxDecorationWithRoundedCorners(
          borderRadius: const BorderRadius.all(Radius.circular(20)),
          border: Border.all(color: Colors.orange),
          backgroundColor: false ? cardDarkColor : const Color(0xFFF8F8F8),
        ),
        padding: const EdgeInsets.fromLTRB(8.0, 0, 8.0, 0),
        child: DropdownButton<String>(
          value: ashrayaLevelValue,
          elevation: 16,
          style: primaryTextStyle(),
          hint: Text('Select Ashraya Level', style: primaryTextStyle()),
          isExpanded: true,
          underline: Container(
            height: 0,
            color: Colors.deepPurpleAccent,
                    ...
                    onPressed: () async {
                      try {
                        await FirebaseFirestore.instance
                          ...
                          "Ashraya_Level": ashrayaLevelValue,
                          "Gift-Center": giftCenterValue,
                          "Preacher": preacherValue,
                          "DOA": userDateOfAnniverserryController.text
                        });
                        await FirebaseFirestore.instance.collection("Japa").doc("${FirebaseAuth.instance.currentUser!.uid}").set({
                                                    "Ashraya_Level": ashrayaLevelValue,
    ...
    return AppBar(
      ...
      leading: showBack
          ? IconButton(
              ...
              icon: const Icon(Icons.arrow_back, color: false ? white : black),
            )
          : null,
      title:
          appBarTitleWidget(context, title, textColor: textColor, color: Colors.transparent),
      actions: actions,
      elevation: 0.5,
    );
  }
  ...
  InputDecoration inputDecoration(
    ...
    return InputDecoration(
      ...
      filled: true,
      fillColor: false ? cardDarkColor : const Color(0xFFF8F8F8),
    );
  }
}
